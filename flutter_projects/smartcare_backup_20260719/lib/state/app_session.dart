import 'package:flutter/foundation.dart';

enum UserRole {
  patient,
  doctor,
  secretary;

  String get label {
    switch (this) {
      case UserRole.patient:
        return 'Patient';
      case UserRole.doctor:
        return 'Doctor';
      case UserRole.secretary:
        return 'Secretary';
    }
  }

  String get dashboardTitle {
    switch (this) {
      case UserRole.patient:
        return 'Patient Dashboard';
      case UserRole.doctor:
        return 'Doctor Dashboard';
      case UserRole.secretary:
        return 'Secretary Dashboard';
    }
  }

  String get dashboardRoute {
    switch (this) {
      case UserRole.patient:
        return '/dashboard';
      case UserRole.doctor:
        return '/doctor-dashboard';
      case UserRole.secretary:
        return '/secretary-dashboard';
    }
  }

  String get displayName {
    switch (this) {
      case UserRole.patient:
        return 'Carise S. Pineda';
      case UserRole.doctor:
        return 'Dr. R. Lim';
      case UserRole.secretary:
        return 'Carise S. Pineda';
    }
  }

  String get initials {
    switch (this) {
      case UserRole.patient:
        return 'CP';
      case UserRole.doctor:
        return 'RL';
      case UserRole.secretary:
        return 'CP';
    }
  }

  String get profileDetail {
    switch (this) {
      case UserRole.patient:
        return 'Patient ID: 09283744';
      case UserRole.doctor:
        return 'Doctor ID: DR-1028';
      case UserRole.secretary:
        return 'Secretary ID: SC-0214';
    }
  }

  bool get canSwitchStaffRole {
    return this == UserRole.doctor || this == UserRole.secretary;
  }
}

class SmartCareSession {
  SmartCareSession._();

  static final Map<String, String> personalInfo = {
    "Email": "mariasantos@gmail.com",
    "Phone": "+6391234567890",
    "Birthday": "March 12, 1990",
    "Address": "Bulaon, San Fernando, Pampanga, Philippines",
  };

  static final Map<String, String> medicalInfo = {
    "Allergies": "None",
    "Medical Condition": "Hypertension",
    "Current Medication": "Paracetamol",
    "Emergency Contact": "Noynoy Aquino",
    "Current Doctor": "Bert Ocampo",
  };

  static final ValueNotifier<UserRole> role = ValueNotifier<UserRole>(
    UserRole.patient,
  );

  static int? currentUserId;
  static String? currentUserEmail;
  static String? currentUserFirstName;
  static String? currentUserLastName;

  static UserRole get currentRole => role.value;

  static void switchRole(UserRole nextRole) {
    if (role.value == nextRole) {
      return;
    }

    role.value = nextRole;
  }

  static void setCurrentUser(Map<String, dynamic> user) {
    currentUserId = user['user_id'] is int
        ? user['user_id'] as int
        : int.tryParse('${user['user_id']}');
    currentUserEmail = user['email'] as String?;
    currentUserFirstName = user['first_name'] as String?;
    currentUserLastName = user['last_name'] as String?;

    switch (user['role']) {
      case 'doctor':
        switchRole(UserRole.doctor);
        break;
      case 'secretary':
        switchRole(UserRole.secretary);
        break;
      default:
        switchRole(UserRole.patient);
    }
  }

  static void updatePersonalInfo(String field, String value) {
    personalInfo[field] = value;
  }

  static void updateMedicalInfo(String field, String value) {
    medicalInfo[field] = value;
  }
}
