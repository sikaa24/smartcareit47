...
import 'package:smartcare/profile.dart';
...
import 'package:smartcare/screens/secretary_dashboard_screen.dart';
import 'package:smartcare/screens/secretary_panel_screen.dart';
...
void main() {
  ...
  testWidgets('secretary dashboard flow opens the management panel', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
        home: const SecretaryDashboardScreen(),
        routes: {
          '/secretary-panel': (_) =>
              const Scaffold(body: Center(child: Text('Secretary panel'))),
          '/queue': (_) =>
              const Scaffold(body: Center(child: Text('Queue target'))),
        },
      ),
    ...
    expect(find.text('Secretary panel'), findsOneWidget);
  });

  testWidgets('secretary panel supports the secretary flowchart branches', (
    WidgetTester tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(
        home: const SecretaryPanelScreen(),
        routes: {
          '/queue': (_) =>
              const Scaffold(body: Center(child: Text('Queue target'))),
        },
    ...
    await tester.ensureVisible(find.text('Input Walk-in Patient'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('Input Walk-in Patient'));
    await tester.pumpAndSettle();

    await tester.ensureVisible(find.text('Walk-in Patient 1'));
    await tester.pumpAndSettle();
    expect(find.text('Walk-in Patient 1'), findsOneWidget);

    await tester.ensureVisible(find.text('Patient Records'));
    await tester.pumpAndSettle();
    expect(find.text('Patient Records'), findsOneWidget);

    await tester.ensureVisible(find.text('Update Record').first);
    await tester.pumpAndSettle();
    ...
    await tester.ensureVisible(find.text('Send Appointment Notifications'));
    await tester.pumpAndSettle();
    await tester.tap(find.text('Send Appointment Notifications'));
    await tester.pumpAndSettle();

    expect(find.text('2 appointment notifications sent.'), findsOneWidget);

    await tester.ensureVisible(find.text('View Availability').first);
    await tester.pumpAndSettle();
    await tester.ensureVisible(find.text('Check Geofence').first);
    await tester.pumpAndSettle();

    expect(find.text('View Availability'), findsWidgets);
    expect(find.text('Check Geofence'), findsWidgets);
  });
